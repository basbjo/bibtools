  ...
  \begin{itemize}
    ...
    \item Formatting, filename and citation key
      \begin{tabbing}
        ...
        \alert<1>{\texttt{newbib}} \> Combine the 3 above tasks \\
      \end{tabbing}
...
\begin{frame}[fragile]{Adding new pdf+bibtex files (\texttt{newbib})}
  \begin{itemize}
    ...
    \item \emph{Format, rename} and \emph{create cite key} for a bib and pdf file\\
      ...
      \begin{lstlisting}
        $ newbib article.pdf article.bib
        $ newbib article.pdf
        $ newbib
	$ newbib -l # select newest files in directory
      \end{lstlisting}

    \item The same with a \emph{target directory} other than \verb!new_lit!
      \begin{lstlisting}
        $ newbib imaging article.pdf
      \end{lstlisting}
...
\begin{frame}[fragile]{Step by step (what \texttt{newbib} does)}
  \begin{tabbing}
    Note: \=use option {\faint\verb!-n!} to only view changes
	  ...
	  are also supported by \texttt{newbib}
  \end{tabbing}
...
\begin{frame}{Ambiguous keys (\texttt{bib-keyinsert})}
  \begin{itemize}[<+->]
...
\begin{frame}{Ambiguous keys (\texttt{bib-name})}
  \begin{itemize}[<+->]
    \item The issue of \emph{non unique} citation keys
